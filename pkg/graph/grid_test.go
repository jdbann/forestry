...
func TestGridGraph_FindNeighbours(t *testing.T) {
	type testCase struct {
		name             string
		width, height    int
		target           geo.Point
		expectNeighbours []geo.Point
	}

	run := func(t *testing.T, tc testCase) {
		g := graph.NewGridGraph(tc.width, tc.height)

		neighbours := g.FindNeighbours(tc.target)
		assert.DeepEqual(t, tc.expectNeighbours, neighbours)
	}

	testCases := []testCase{
		{
			name:   "3x3 grid corner",
			width:  3,
			height: 3,
			target: geo.Point{X: 0, Y: 0},
			expectNeighbours: []geo.Point{
				{X: 1, Y: 0},
				{X: 0, Y: 1},
			},
		...
		{
			name:   "3x3 grid centre",
			width:  3,
			height: 3,
			target: geo.Point{X: 1, Y: 1},
			expectNeighbours: []geo.Point{
				{X: 0, Y: 1},
				{X: 1, Y: 0},
				{X: 2, Y: 1},
...
func TestGridGraph_FindPath(t *testing.T) {
	type testCase struct {
		name          string
		width, height int
		from, to      geo.Point
